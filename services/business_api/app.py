...
from .publishing_context import publishing_context
...
class BusinessApi:
    def __init__(self, database: Database | None = None, session_ttl: timedelta = timedelta(hours=8),
                 publishing_token: str | None = None) -> None:
        self.database = database or Database()
        self.session_ttl = session_ttl
        self.sessions: dict[str, tuple[str, datetime]] = {}
        self.publishing_token = publishing_token
    ...
    def _request(self, method, path, headers=None, body=None):
        ...
        try:
            if method == "GET" and route.startswith("/api/v1/internal/tasks/") and route.endswith("/publishing-context"):
                authorization = headers.get("authorization", "")
                expected = f"Bearer {self.publishing_token}" if self.publishing_token else ""
                if not expected or not secrets.compare_digest(authorization.encode(), expected.encode()):
                    return Response(401, {"error": "publishing service credential required"})
                if len(route.split("/")) != 7:
                    return Response(404, {"error": "not found"})
                try:
                    return Response(200, publishing_context(self.database.connection, route.split("/")[5]))
                except KeyError:
                    return Response(404, {"error": "task not found"})
                except (ValueError, sqlite3.OperationalError):
                    return Response(409, {"error": "publishing context unavailable"})
