...
- [x] 共享契约及模块全部自动化测试通过。
- [x] 本地真实 HTTP 联调：业务 API 提供可信上下文，工作流消费事件并实际转 PAUSED；8 项集成测试通过，见 integration-plan.md。
- [ ] 前端与部署环境联调：前端使用真实指标响应，部署配置、凭据和网络由团队环境验证。
...
边界：SQLite 事务保证模拟逻辑结果唯一，崩溃回滚可重新计算无副作用的模拟结果。真实发布平台需要额外幂等与对账设计。后续联调修复已实现业务上下文与工作流适配器并通过本地真实 HTTP 测试；前端和部署环境仍待验收，不能据此宣称 Issue 全部完成。
