...
class Workflow:
    ...
    def receive(self, event):
        ...
        with self.atomic():
            ...
            if task["status"] == "PUBLISHING" and event["event_type"] in {"task.published.v1", "task.failed.v1"}:
                if event["idempotency_key"] != task["active_request_key"]:
                    raise ValueError("publication result must match active request key")
    ...
    def _result(self, task, event_type, data):
        if event_type == "task.script_reviewed.v1":
            ...
            if stage == "publish":
                if (data.get("failed_step") != "publishing"
                        or data.get("request_key") != task["active_request_key"]
                        or data.get("receipt_id") != task["receipt_id"]
                        or data.get("retryable") is not False
                        or type(data.get("attempt")) is not int or not 0 <= data["attempt"] <= 2):
                    raise ValueError("invalid final publication failure")
                task["publication_attempts"] = data["attempt"]
                task["retry_at"] = None
            ...
            for task in tasks:
                ...
                if task["status"] == "FAILED" and task["retry_at"] and now >= datetime.fromisoformat(task["retry_at"]):
                    ...
                    if stage == "media":
                        ...
                        self._move(task, "PAUSED", "legacy publication retry requires receipt reconciliation")
                    else:
                        self._script(task)
    ...
    def _publish(self, task):
        self._emit("task.publish_requested.v1", {"task_id": task["task_id"], "receipt_id": task["receipt_id"],
                   ...
                   f"{task['task_id']}:publish:{task['receipt_id']}")
        task.update(self.inspect(task["task_id"]))
