...
or copied into this change. There is no publishing service implementation yet.
...
- External operations get three retries after the initial call, waiting
  1, 5 and 15 minutes after each failure. Publishing gets only one retry after
  one minute, with the same receipt ID. This remains a provisional publishing
  adapter contract: request keys identify attempts, receipt ID identifies the
  single logical publication. Member 6 must agree before live integration.
- Failure payloads must include `request_key` for the active request and a
  zero-based `attempt`; stale attempts cannot consume a new retry budget.
...
- Content generation/results, publishing execution, user-facing workflow HTTP
